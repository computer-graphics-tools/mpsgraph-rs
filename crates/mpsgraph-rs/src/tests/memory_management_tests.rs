...
fn test_graph_cloning_and_dropping() {
    ...
    for (i, clone) in clones.iter().enumerate() {
        ...
        let _tensor = clone.placeholder(&shape, MPSDataType::Float32, Some(&format!("tensor_{}", i)));
    }
...
fn test_multithreaded_tensor_access() {
    ...
    let shape = MPSShape::from_slice(&[2, 2]);
    let tensor = Arc::new(Mutex::new(graph.placeholder(&shape, MPSDataType::Float32, Some("shared"))));
...
fn test_large_number_of_tensors() {
    ...
    // Using a scope to control lifetimes
    {
        ...
        let tensors: Vec<_> = (0..100)
            .map(|i| graph.placeholder(&shape, MPSDataType::Float32, Some(&format!("tensor_{}", i))))
            .collect();
