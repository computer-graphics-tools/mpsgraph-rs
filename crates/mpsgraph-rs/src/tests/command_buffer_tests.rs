...
fn test_graph_execution() {
    ...
    if let Some(metal_device) = metal::Device::system_default() {
        ...
        let a_tensor = TensorBuffer::new(&metal_device, &[1.0f32, 2.0, 3.0, 4.0], &[2, 2], MPSDataType::Float32);
        let b_tensor = TensorBuffer::new(&metal_device, &[5.0f32, 6.0, 7.0, 8.0], &[2, 2], MPSDataType::Float32);
        
        // Create output tensor buffer (we don't actually need to use this in the test)
        let _result_tensor = TensorBuffer::new(&metal_device, &[0.0f32, 0.0, 0.0, 0.0], &[2, 2], MPSDataType::Float32);
        ...
        // Verify results
        assert!(results.contains_key(&result), "Results should contain the output tensor");
    }
}
...
fn test_graph_reuse() {
    ...
    if let Some(metal_device) = metal::Device::system_default() {
        ...
        // Create input data 1
        let a_tensor1 = TensorBuffer::new(&metal_device, &[1.0f32, 2.0, 3.0, 4.0], &[2, 2], MPSDataType::Float32);
        ...
        let a_tensor2 = TensorBuffer::new(&metal_device, &[5.0f32, 6.0, 7.0, 8.0], &[2, 2], MPSDataType::Float32);
